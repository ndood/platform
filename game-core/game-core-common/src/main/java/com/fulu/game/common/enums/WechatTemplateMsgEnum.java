...
public enum WechatTemplateMsgEnum {
    ...
    ORDER_TOUSER_REJECT_RECEIVE(WechatPagePathEnum.ORDER_LIST_PAGE, "陪玩师太忙了，为了不耽误您的游戏体验，拒绝了订单，望老板大人理解。"),//拒绝接单 A

    ORDER_TOUSER_AFFIRM_RECEIVE(WechatPagePathEnum.ORDER_LIST_PAGE, "陪玩师已经接单啦，一起谈谈什么时候开始呢？超过24小时未开始，订单将自动取消。"),//已经接单 A
    ORDER_TOUSER_START_SERVICE(WechatPagePathEnum.ORDER_LIST_PAGE, "陪玩师开始陪玩服务啦，快快上号开始吧。"), //开始服务 A
    ORDER_TOUSER_CHECK(WechatPagePathEnum.ORDER_LIST_PAGE, "陪玩师向发起了完成服务申请，快去确认吧。"), //提交验收 A
    ORDER_TOUSER_APPEAL(WechatPagePathEnum.ORDER_LIST_PAGE, "对于您的协商，陪玩师申请客服进行仲裁，快去提交凭证吧（2小时内提交）。"), //用户发起仲裁 A
    ...
    ORDER_TOUSER_CONSULT_REJECT(WechatPagePathEnum.ORDER_LIST_PAGE, "对于您的协商，陪玩师进行了拒绝，快去处理吧，超过2小时未处理，将默认取消协商。"), //拒绝协商 A
    ORDER_TOUSER_CONSULT_AGREE(WechatPagePathEnum.ORDER_LIST_PAGE, "对于您的协商，陪玩师进行了同意，金额将退回到您的微信账号。"), //同意协商 A

    ORDER_TOSERVICE_PAY(WechatPagePathEnum.ORDER_LIST_PAGE, "有老板向您下单啦，快去查看吧，超过24小时未接单，订单将自动取消。"), //下单
    ORDER_TOSERVICE_REMIND_RECEIVE(WechatPagePathEnum.ORDER_LIST_PAGE, "老板提醒您接单，快去处理吧，超过24小时未接单，订单将自动取消。"), //提醒接单 A
    ORDER_TOSERVICE_REMIND_START_SERVICE(WechatPagePathEnum.ORDER_LIST_PAGE, "老板提醒您开始服务，快去处理吧，超过指定时间未开始，订单将自动取消。"), //提醒开始 A
    ORDER_TOSERVICE_AFFIRM_SERVER(WechatPagePathEnum.ORDER_LIST_PAGE, "老板已确认服务完成，收益到账了哦。"), //确认服务完成 A
    ORDER_TOSERVICE_ORDER_CANCEL(WechatPagePathEnum.ORDER_LIST_PAGE, "很遗憾，老板取消了订单，要再接再厉哦。"), //确认服务完成 A
    ORDER_TOSERVICE_CONSULT(WechatPagePathEnum.ORDER_LIST_PAGE, "老板发起协商，申请退款，快去处理吧，超过24小时未处理，将默认同意协商。"), //确认服务完成 A
    ORDER_TOSERVICE_APPEAL(WechatPagePathEnum.ORDER_LIST_PAGE, "老板对您的回馈不满意，申请了客服仲裁，快去提交凭证吧（2小时内提交）。"), //确认服务完成 A
    ...
    ORDER_TOSERVICE_CONSULT_CANCEL(WechatPagePathEnum.ORDER_LIST_PAGE, "老板取消了协商，服务照常进行哦。"), //确认服务完成 A
