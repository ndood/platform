...
public class UserAutoReceiveOrder implements Serializable {
    ...
    //用户自己自动接单设置
    @Excel(name = "自动接单设置", orderNum = "4", width = 15, replace = {"开_true",
            "关_false"})
    ...
    //接单范围开始
    @Excel(name = "接单范围开始", orderNum = "9", width = 15)
    private Integer startRank;
    //接单范围结束
    @Excel(name = "接单范围结束", orderNum = "10", width = 15)
    ...
    //自动派单完成数
    @Excel(name = "上分订单完成数", orderNum = "11", width = 15)
    private Integer orderCompleteNum;
    //派单取消数
    @Excel(name = "上分订单取消数", orderNum = "12", width = 15)
    private Integer orderCancelNum;
    //派单协商仲裁数
    @Excel(name = "上分订单仲裁、协商数", orderNum = "13", width = 20)
